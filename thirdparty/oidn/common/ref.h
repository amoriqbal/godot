...
public:
    ...
    __forceinline Ref ( const Ref& other ) noexcept : ptr ( other.ptr )
    {
        if ( ptr ) {
            ptr->incRef();
        }
    }
    __forceinline Ref ( Ref&& other ) noexcept : ptr ( other.ptr )
    {
        other.ptr = nullptr;
    }
    __forceinline Ref ( T* ptr ) noexcept : ptr ( ptr )
    {
        if ( ptr ) {
            ptr->incRef();
        }
    ...
    template<typename Y>
    __forceinline Ref ( const Ref<Y>& other ) noexcept : ptr ( other.get() )
    {
        if ( ptr ) {
            ptr->incRef();
        }
    ...
    template<typename Y>
    __forceinline explicit Ref ( Y* ptr ) noexcept : ptr ( ptr )
    {
        if ( ptr ) {
            ptr->incRef();
        }
    ...
    __forceinline T* get() const noexcept
    {
        return ptr;
    }
...
template<typename T> __forceinline bool operator == ( const Ref<T>& a, std::nullptr_t )  noexcept
{
    return a.ptr   == nullptr;
}
template<typename T> __forceinline bool operator == ( std::nullptr_t,  const Ref<T>& b ) noexcept
{
