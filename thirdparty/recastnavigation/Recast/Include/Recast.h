...
public:
    ...
    inline void enableLog ( bool state )
    {
        m_logEnabled = state;
    }
    ...
    inline void enableTimer ( bool state )
    {
    ...
    inline void startTimer ( const rcTimerLabel label )
    {
        if ( m_timerEnabled ) {
            doStartTimer ( label );
        }
    ...
    ///  @param	label	The category of the timer.
    inline void stopTimer ( const rcTimerLabel label )
    {
        if ( m_timerEnabled ) {
            doStopTimer ( label );
        }
    ...
    inline int getAccumulatedTime ( const rcTimerLabel label ) const
    {
...
public:
    ...
    ///  @param[in]		label	The category of the timer.
    inline rcScopedTimer ( rcContext* ctx, const rcTimerLabel label ) : m_ctx ( ctx ), m_label ( label )
    {
...
struct rcSpan {
unsigned int smin :
    RC_SPAN_HEIGHT_BITS; ///< The lower limit of the span. [Limit: < #smax]
unsigned int smax :
    RC_SPAN_HEIGHT_BITS; ///< The upper limit of the span. [Limit: <= #RC_SPAN_MAX_HEIGHT]
...
template<class T> inline void rcSwap ( T& a, T& b )
{
    T t = a;
    a = b;
    b = t;
...
template<class T> inline T rcMin ( T a, T b )
{
    return a < b ? a : b;
}
...
template<class T> inline T rcMax ( T a, T b )
{
    return a > b ? a : b;
}
...
template<class T> inline T rcAbs ( T a )
{
    return a < 0 ? -a : a;
}
...
template<class T> inline T rcSqr ( T a )
{
    return a*a;
}
...
template<class T> inline T rcClamp ( T v, T mn, T mx )
{
