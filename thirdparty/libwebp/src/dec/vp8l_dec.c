...
static int Is8bOptimizable ( const VP8LMetadata* const hdr )
{
    ...
    for ( i = 0; i < hdr->num_htree_groups_; ++i ) {
        ...
        if ( htrees[RED][0].bits > 0 ) {
            return 0;
        }
        if ( htrees[BLUE][0].bits > 0 ) {
            return 0;
        }
        if ( htrees[ALPHA][0].bits > 0 ) {
            return 0;
        }
    }
    return 1;
...
End:
    if ( !ok ) {
        ...
        if ( !is_level0 ) {
            ClearMetadata ( hdr );    // Clean up temporary data behind.
        }
    }
    return ok;
}
...
int VP8LDecodeHeader ( VP8LDecoder* const dec, VP8Io* const io )
{
    ...
    if ( !DecodeImageStream ( width, height, 1, dec, NULL ) ) {
        goto Error;
    }
    return 1;
...
int VP8LDecodeImage ( VP8LDecoder* const dec )
{
    ...
    if ( dec->state_ != READ_DATA ) {
        ...
        if ( !AllocateInternalBuffers32b ( dec, io->width ) ) {
            goto Err;
        }
