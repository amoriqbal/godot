...
static int AddFrame ( WebPDemuxer* const dmux, Frame* const frame )
{
    ...
    if ( last_frame != NULL && !last_frame->complete_ ) {
        return 0;
    }
                                ...
                                MemBuffer* const mem, Frame* const frame )
{
    ...
    do {
        ...
        if ( SizeIsInvalid ( mem, payload_size_padded ) ) {
            return PARSE_ERROR;
        }
...
static ParseStatus ParseVP8XChunks ( WebPDemuxer* const dmux )
{
    ...
    do {
        ...
        if ( SizeIsInvalid ( mem, chunk_size_padded ) ) {
            return PARSE_ERROR;
        }
...
static ParseStatus ParseVP8X ( WebPDemuxer* const dmux )
{
    ...
    if ( SizeIsInvalid ( mem, vp8x_size ) ) {
        return PARSE_ERROR;
    }
    if ( MemDataSize ( mem ) < vp8x_size ) {
        return PARSE_NEED_MORE_DATA;
    }
    ...
    if ( SizeIsInvalid ( mem, CHUNK_HEADER_SIZE ) ) {
        return PARSE_ERROR;
    }
    if ( MemDataSize ( mem ) < CHUNK_HEADER_SIZE ) {
        return PARSE_NEED_MORE_DATA;
...
static int IsValidSimpleFormat ( const WebPDemuxer* const dmux )
{
    ...
    if ( dmux->canvas_width_ <= 0 || dmux->canvas_height_ <= 0 ) {
        return 0;
    }
    if ( dmux->state_ == WEBP_DEMUX_DONE && frame == NULL ) {
        return 0;
    }
...
static int IsValidExtendedFormat ( const WebPDemuxer* const dmux )
{
    ...
    if ( dmux->state_ == WEBP_DEMUX_DONE && dmux->frames_ == NULL ) {
        return 0;
    }
    if ( dmux->feature_flags_ & ~ALL_VALID_FLAGS ) {
    ...
    while ( f != NULL ) {
        ...
        for ( ; f != NULL && f->frame_num_ == cur_frame_set; f = f->next_ ) {
            ...
            if ( !is_animation && f->frame_num_ > 1 ) {
                return 0;
            }
            ...
            if ( f->complete_ ) {
                ...
                if ( dmux->state_ == WEBP_DEMUX_DONE ) {
                    return 0;
                }
                      ...
                      WebPChunkIterator* const iter )
{
    ...
    if ( dmux == NULL || fourcc == NULL || chunk_num < 0 ) {
        return 0;
    }
