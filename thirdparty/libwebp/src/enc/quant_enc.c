                             ...
                             const VP8ModeScore* const rd )
{
    ...
    for ( j = 0; j < 16; ++j ) {
        for ( i = 0; i < 16; ++i ) {
            printf ( "%3d ", y_in[i + j * BPS] );
        }
        printf ( "     " );
        for ( i = 0; i < 16; ++i ) {
            printf ( "%3d ", y_out[i + j * BPS] );
        }
        printf ( "     " );
        for ( i = 0; i < 16; ++i ) {
    ...
    if ( is_i16 ) {
        ...
        printf ( "y_dc_levels:" );
        for ( i = 0; i < 16; ++i ) {
            printf ( "%3d ", rd->y_dc_levels[i] );
        }
        printf ( "\n" );
    } else {
        printf ( "Modes[16]: " );
        for ( i = 0; i < 16; ++i ) {
            printf ( "%d ", rd->modes_i4[i] );
        }
        printf ( "\n" );
    }
    printf ( "y_ac_levels:\n" );
...
static void SimplifySegments ( VP8Encoder* const enc )
{
    ...
    if ( num_final_segments < num_segments ) { // Remap
        ...
        while ( i-- > 0 ) {
            enc->mb_info_[i].segment_ = map[enc->mb_info_[i].segment_];
        }
