                            ...
                            int type, float results[5] )
{
    ...
    w = src->width;
    h = src->height;
    if ( !WebPPictureView ( src, 0, 0, w, h, &p0 ) ) {
        goto Error;
    }
    if ( !WebPPictureView ( ref, 0, 0, w, h, &p1 ) ) {
        goto Error;
    }

    // We always measure distortion in ARGB space.
    if ( p0.use_argb == 0 && !WebPPictureYUVAToARGB ( &p0 ) ) {
        goto Error;
    }
    if ( p1.use_argb == 0 && !WebPPictureYUVAToARGB ( &p1 ) ) {
        goto Error;
    }
    for ( c = 0; c < 4; ++c ) {
        float distortion;
                          ...
                          int type, float* distortion, float* result )
{
    ...
    ( void ) x_step;
    ( void ) type;
    if ( distortion == NULL || result == NULL ) {
        return 0;
    }
    *distortion = 0.f;
    *result = 0.f;
