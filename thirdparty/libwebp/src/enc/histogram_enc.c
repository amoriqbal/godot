        ...
        int low_effort )
{
    ...
    for ( idx = 0; idx < *num_used; ++idx ) {
        cluster_mappings[idx] = idx;
    }
                                    ...
                                    int* const num_used )
{
    ...
    for ( i = 0; i < image_histo_size; ++i ) {
        if ( image_histo->histograms[i] == NULL ) {
            continue;
        }
        for ( j = i + 1; j < image_histo_size; ++j ) {
            // Initialize queue.
            if ( image_histo->histograms[j] == NULL ) {
                continue;
            }
    ...
    while ( histo_queue.size > 0 ) {
        ...
        for ( i = 0; i < image_histo->size; ++i ) {
            if ( i == idx1 || image_histo->histograms[i] == NULL ) {
                continue;
            }
                                        ...
                                        int* const do_greedy )
{
    ...
    if ( !HistoQueueInit ( &histo_queue, kHistoQueueSize ) ) {
        goto End;
    }
                                       ...
                                       uint16_t* const symbols )
{
    ...
    for ( i = 0; i < set->max_size; ++i ) {
        int cluster;
        if ( symbols[i] == kInvalidHistogramSymbol ) {
            continue;
        }
