                               ...
                               int copy_data, uint32_t tag )
{
    ...
    if ( data != NULL ) {
        if ( copy_data ) {      // Copy data.
            if ( !WebPDataCopy ( data, &chunk->data_ ) ) {
    ...
    for ( current = wpi_list; current != NULL; current = current->next_ ) {
        if ( id == WEBP_CHUNK_NIL ) {
            ...
            if ( wpi_chunk != NULL ) {
                ...
                if ( wpi_chunk_id == id ) {
                    ++count;    // Count images with a matching 'id'.
                }
            }
        }
    }
...
size_t MuxImageDiskSize ( const WebPMuxImage* const wpi )
{
    size_t size = 0;
    if ( wpi->header_ != NULL ) {
        size += ChunkDiskSize ( wpi->header_ );
    }
    if ( wpi->alpha_ != NULL ) {
        size += ChunkDiskSize ( wpi->alpha_ );
    }
    if ( wpi->img_ != NULL ) {
        size += ChunkDiskSize ( wpi->img_ );
    }
    if ( wpi->unknown_ != NULL ) {
        size += ChunkListDiskSize ( wpi->unknown_ );
    }
    return size;
}
