                               ...
                               WebPData* const full_image )
{
    ...
    if ( !DecodeFrameOntoCanvas ( frame, canvas_buf ) ) {
        goto Err;
    }
    if ( !EncodeFrame ( &enc->last_config_, canvas_buf, &mem1 ) ) {
        goto Err;
    }
    GetEncodedData ( &mem1, full_image );

    if ( enc->options_.allow_mixed ) {
        if ( !EncodeFrame ( &enc->last_config_reversed_, canvas_buf, &mem2 ) ) {
            goto Err;
        }
        ...
        WebPData* const webp_data )
{
    ...
    if ( frame.id != WEBP_CHUNK_ANMF ) {
        goto End;    // Non-animation: nothing to do.
    }
