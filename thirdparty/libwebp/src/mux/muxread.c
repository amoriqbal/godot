                           ...
                           WebPMuxImage* const wpi )
{
    ...
    while ( bytes != last ) {
        ...
        case WEBP_CHUNK_ALPHA:
            if ( wpi->alpha_ != NULL ) {
                goto Fail;    // Consecutive ALPH chunks.
            }
            if ( ChunkSetHead ( &subchunk, &wpi->alpha_ ) != WEBP_MUX_OK ) {
                goto Fail;
            }
        ...
        case WEBP_CHUNK_IMAGE:
            if ( wpi->img_ != NULL ) {
                goto Fail;    // Only 1 image chunk allowed.
            }
            if ( ChunkSetHead ( &subchunk, &wpi->img_ ) != WEBP_MUX_OK ) {
                goto Fail;
            }
            if ( !MuxImageFinalize ( wpi ) ) {
                goto Fail;
            }
                                 ...
                                 int version )
{
    ...
    while ( data != end ) {
        ...
        case WEBP_CHUNK_ALPHA:
            if ( wpi->alpha_ != NULL ) {
                goto Err;    // Consecutive ALPH chunks.
            }
            if ( ChunkSetHead ( &chunk, &wpi->alpha_ ) != WEBP_MUX_OK ) {
                goto Err;
            }
            wpi->is_partial_ = 1;  // Waiting for a VP8 chunk.
            break;
        case WEBP_CHUNK_IMAGE:
            if ( ChunkSetHead ( &chunk, &wpi->img_ ) != WEBP_MUX_OK ) {
                goto Err;
            }
            if ( !MuxImageFinalize ( wpi ) ) {
                goto Err;
        ...
        PushImage:
            ...
            if ( MuxImagePush ( wpi, &mux->images_ ) != WEBP_MUX_OK ) {
                goto Err;
            }
        ...
        case WEBP_CHUNK_ANMF:
            if ( wpi->is_partial_ ) {
                goto Err;    // Previous wpi is still incomplete.
            }
            if ( !MuxImageParse ( &chunk, copy_data, wpi ) ) {
                goto Err;
            }
            ChunkRelease ( &chunk );
            goto PushImage;
    ...
    if ( MuxValidate ( mux ) != WEBP_MUX_OK ) {
        goto Err;
    }
                                       ...
                                       int* width, int* height, uint32_t* flags )
{
    ...
    if ( w * ( uint64_t ) h >= MAX_IMAGE_AREA ) {
        return WEBP_MUX_BAD_DATA;
    }
        ...
        WebPMuxFrameInfo* const frame )
{
    ...
    if ( frame_data->size < kChunks[IDX_ANMF].size ) {
        return WEBP_MUX_BAD_DATA;
    }
    // Extract info.
        ...
        WebPMuxAnimParams* params )
{
    ...
    if ( anim.size < kChunks[WEBP_CHUNK_ANIM].size ) {
        return WEBP_MUX_BAD_DATA;
    }
