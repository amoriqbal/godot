...
struct pixel_block {
    ...
    const color_rgba &operator() ( uint32_t x, uint32_t y ) const
    {
        assert ( ( x < cPixelBlockWidth ) && ( y < cPixelBlockHeight ) );
        return m_pixels[y][x];
    }
    color_rgba &operator() ( uint32_t x, uint32_t y )
    {
        assert ( ( x < cPixelBlockWidth ) && ( y < cPixelBlockHeight ) );
        return m_pixels[y][x];
    }

    const color_rgba *get_ptr() const
    {
        return &m_pixels[0][0];
    }
    color_rgba *get_ptr()
