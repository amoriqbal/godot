...
{
    return ( i < 0 ) ? static_cast<uint32_t> ( -i ) : static_cast<uint32_t> ( i );
}
...
template<typename S> inline S square ( S val )
{
    return val * val;
}
...
struct packed_uint {
    ...
    inline packed_uint()
    {
        static_assert ( NumBytes <= 4, "NumBytes <= 4" );
    }
    inline packed_uint ( uint32_t v )
    {
        *this = v;
    }
    ...
    inline operator uint32_t() const
    {
        ...
        default:
            return ( m_bytes[3] << 24U ) | ( m_bytes[2] << 16U ) | ( m_bytes[1] << 8U ) | ( m_bytes[0] );
        }
    }
};
