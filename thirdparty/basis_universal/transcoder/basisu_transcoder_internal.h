...
public:
    ...
    const basisu::uint8_vec &get_code_sizes() const
    {
        return m_code_sizes;
    }
...
public:
    ...
    const basisu::int_vec& get_values() const
    {
        return m_values;
    }
    basisu::int_vec& get_values()
    {
        return m_values;
    }
    ...
    void init_flags()
    {
        ...
        for ( uint32_t i = 0; i < 4; i++ ) {
            if ( hist[i] ) {
                ...
                if ( i < m_lo_selector ) {
                    m_lo_selector = static_cast<uint8_t> ( i );
                }
                if ( i > m_hi_selector ) {
                    m_hi_selector = static_cast<uint8_t> ( i );
                }
            }
        }
    }
