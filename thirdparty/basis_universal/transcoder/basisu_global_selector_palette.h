...
struct etc1_global_selector_codebook_entry_id {
    ...
    void set ( uint32_t palette_index, const etc1_global_palette_entry_modifier &modifier )
    {
...
public:
    ...
    etc1_global_selector_codebook ( uint32_t N, const uint32_t *pEntries )
    {
        init ( N, pEntries );
    }
    ...
    uint32_t size() const
    {
        return ( uint32_t ) m_palette.size();
    }
