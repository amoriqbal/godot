...
public:
    ...
    inline vec ( T v0, T v1 )
    {
        set ( v0, v1 );
    }
    inline vec ( T v0, T v1, T v2 )
    {
        set ( v0, v1, v2 );
    }
    inline vec ( T v0, T v1, T v2, T v3 )
    {
        set ( v0, v1, v2, v3 );
    }
    ...
    template <uint32_t OtherN, typename OtherT> inline vec ( const vec<OtherN, OtherT> &other )
    {
        set ( other );
    }
    ...
    inline T getY() const
    {
        static_assert ( N >= 2, "N too small" );
        return m_v[1];
    }
    inline T getZ() const
    {
        static_assert ( N >= 3, "N too small" );
        return m_v[2];
    }
    inline T getW() const
    {
        static_assert ( N >= 4, "N too small" );
        return m_v[3];
    }

    inline bool operator== ( const vec &rhs ) const
    {
        for ( uint32_t i = 0; i < N; i++ ) if ( m_v[i] != rhs.m_v[i] ) {
                return false;
            }
        return true;
    }
    ...
    inline void clear_elements ( uint32_t s, uint32_t e )
    {
        assert ( e <= N );
    ...
    {
        if ( this != &rhs ) for ( uint32_t i = 0; i < N; i++ ) {
                m_v[i] = rhs.m_v[i];
            }
        return *this;
    }
    template <uint32_t OtherN, typename OtherT> inline vec &operator= ( const vec<OtherN, OtherT> &rhs )
    {
        set ( rhs );
        return *this;
    }
    ...
    {
        return reinterpret_cast<const T *> ( &m_v[0] );
    }
    inline T *get_ptr()
    {
        return reinterpret_cast<T *> ( &m_v[0] );
    }
    ...
    static inline T dot_product ( const vec &lhs, const vec &rhs )
    {
        T res = lhs.m_v[0] * rhs.m_v[0];
    ...
    inline T squared_distance ( const vec &other ) const
    {
        T d2 = 0;
        for ( uint32_t i = 0; i < N; i++ ) {
            T d = m_v[i] - other.m_v[i];
            d2 += d * d;
        }
        return d2;
    }
    inline double squared_distance_d ( const vec& other ) const
    {
        double d2 = 0;
        for ( uint32_t i = 0; i < N; i++ ) {
            double d = ( double ) m_v[i] - ( double ) other.m_v[i];
            d2 += d * d;
        }
        return d2;
    }

    inline T distance ( const vec &other ) const
    {
        return static_cast<T> ( sqrt ( squared_distance ( other ) ) );
    }
    inline double distance_d ( const vec& other ) const
    {
        return sqrt ( squared_distance_d ( other ) );
    }

    inline vec &normalize_in_place()
    {
        T len = length();
        if ( len != 0.0f ) {
            *this *= ( 1.0f / len );
...
public:
    ...
    {
        if ( this != &rhs ) for ( uint32_t i = 0; i < Rows; i++ ) {
                m_r[i] = rhs.m_r[i];
            }
        return *this;
    }
...
template<typename T> inline void indirect_sort ( uint32_t num_indices, uint32_t* pIndices, const T* pKeys )
{
    ...
    [pKeys] ( uint32_t a, uint32_t b ) {
        return pKeys[a] < pKeys[b];
    }
    );
}
...
public:
    ...
    inline const uint8_t &operator[] ( uint32_t index ) const
    {
        assert ( index < 4 );
    ...
    {
        if ( m_comps[0] != rhs.m_comps[0] ) {
            return false;
        }
        if ( m_comps[1] != rhs.m_comps[1] ) {
            return false;
        }
        if ( m_comps[2] != rhs.m_comps[2] ) {
            return false;
        }
        if ( m_comps[3] != rhs.m_comps[3] ) {
            return false;
        }
        return true;
    }
    ...
    inline int get_601_luma() const
    {
        return ( 19595U * m_comps[0] + 38470U * m_comps[1] + 7471U * m_comps[2] + 32768U ) >> 16U;
    }
    inline int get_709_luma() const
    {
        return ( 13938U * m_comps[0] + 46869U * m_comps[1] + 4729U * m_comps[2] + 32768U ) >> 16U;
    }
    inline int get_luma ( bool luma_601 ) const
    {
        return luma_601 ? get_601_luma() : get_709_luma();
    }
};
...
public:
    ...
    uint32_t urand32()
    {
        return static_cast<uint32_t> ( irand ( INT32_MIN, INT32_MAX ) );
    }
...
public:
    ...
    inline uint32_t get_top_index() const
    {
        return m_heap[1].m_index;
    }
    inline float get_top_priority() const
    {
        return m_heap[1].m_priority;
    }
...
private:
    ...
    inline void down_heap ( uint32_t heap_index )
    {
        ...
        while ( ( child_index = ( heap_index << 1 ) ) <= m_size ) {
            if ( ( child_index < m_size ) && ( m_heap[child_index].m_priority < m_heap[child_index + 1].m_priority ) ) {
                ++child_index;
            }
...
public:
    ...
    void add_training_vec ( const TrainingVectorType &v, uint64_t weight )
    {
        m_training_vecs.push_back ( std::make_pair ( v, weight ) );
    }

    size_t get_total_training_vecs() const
    {
        return m_training_vecs.size();
    }
    const array_of_weighted_training_vecs &get_training_vecs() const
    {
        return m_training_vecs;
    }
    array_of_weighted_training_vecs &get_training_vecs()
    {
        return m_training_vecs;
    }
...
private:
    ...
    public:
        ...
        // vecs is erased
        inline void set ( const TrainingVectorType &org, uint64_t weight, float var, std::vector<uint32_t> &vecs )
        {
            m_origin = org;
            m_weight = weight;
            m_var = var;
            m_training_vecs.swap ( vecs );
        }

        inline bool is_leaf() const
        {
            return m_left_index < 0;
        }
...
public:
    ...
    inline uint32_t size() const
    {
        return static_cast<uint32_t> ( m_hist.size() );
    }
...
public:
    ...
    inline const uint16_vec &get_codes() const
    {
        return m_codes;
    }
    inline const uint8_vec &get_code_sizes() const
    {
        return m_code_sizes;
    }
...
public:
    ...
    inline const uint8_vec &get_bytes() const
    {
        return m_bytes;
    }

    inline uint64_t get_total_bits() const
    {
        return m_total_bits;
    }
    inline void clear_total_bits()
    {
        m_total_bits = 0;
    }
...
public:
    huff2D() { }
    huff2D ( uint32_t bits_per_sym, uint32_t total_syms_per_group )
    {
        init ( bits_per_sym, total_syms_per_group );
    }
    ...
    inline const huffman_encoding_table &get_encoding_table() const
    {
        return m_encoding_table;
    }
...
public:
    ...
    inline const uint_vec &get_remap_table() const
    {
        return m_remap_table;
    }

private:
    uint_vec m_hist, m_total_count_to_picked, m_entries_picked, m_entries_to_do, m_remap_table;

    inline uint32_t get_hist ( int i, int j, int n ) const
    {
        return ( i > j ) ? m_hist[j * n + i] : m_hist[i * n + j];
    }
    inline void inc_hist ( int i, int j, int n )
    {
        if ( ( i != j ) && ( i < j ) && ( i != -1 ) && ( j != -1 ) ) {
            assert ( ( ( uint32_t ) i < ( uint32_t ) n ) && ( ( uint32_t ) j < ( uint32_t ) n ) );
            m_hist[i * n + j]++;
        }
    }
...
public:
    ...
    inline const color_rgba &operator() ( uint32_t x, uint32_t y ) const
    {
        assert ( x < m_width && y < m_height );
        return m_pixels[x + y * m_pitch];
    }
    inline color_rgba &operator() ( uint32_t x, uint32_t y )
    {
        assert ( x < m_width && y < m_height );
        return m_pixels[x + y * m_pitch];
    }

    inline const color_rgba &get_clamped ( int x, int y ) const
    {
        return ( *this ) ( clamp<int> ( x, 0, m_width - 1 ), clamp<int> ( y, 0, m_height - 1 ) );
    }
    inline color_rgba &get_clamped ( int x, int y )
    {
        return ( *this ) ( clamp<int> ( x, 0, m_width - 1 ), clamp<int> ( y, 0, m_height - 1 ) );
    }
    ...
    inline uint32_t get_block_width ( uint32_t w ) const
    {
        return ( m_width + ( w - 1 ) ) / w;
    }
    inline uint32_t get_block_height ( uint32_t h ) const
    {
        return ( m_height + ( h - 1 ) ) / h;
    }
    inline uint32_t get_total_blocks ( uint32_t w, uint32_t h ) const
    {
        return get_block_width ( w ) * get_block_height ( h );
    }

    inline const color_rgba_vec &get_pixels() const
    {
        return m_pixels;
    }
    inline color_rgba_vec &get_pixels()
    {
        return m_pixels;
    }

    inline const color_rgba *get_ptr() const
    {
        return &m_pixels[0];
    }
    inline color_rgba *get_ptr()
    {
        return &m_pixels[0];
    }
...
public:
    ...
    inline const vec4F &operator() ( uint32_t x, uint32_t y ) const
    {
        assert ( x < m_width && y < m_height );
        return m_pixels[x + y * m_pitch];
    }
    inline vec4F &operator() ( uint32_t x, uint32_t y )
    {
        assert ( x < m_width && y < m_height );
        return m_pixels[x + y * m_pitch];
    }

    inline const vec4F &get_clamped ( int x, int y ) const
    {
        return ( *this ) ( clamp<int> ( x, 0, m_width - 1 ), clamp<int> ( y, 0, m_height - 1 ) );
    }
    inline vec4F &get_clamped ( int x, int y )
    {
        return ( *this ) ( clamp<int> ( x, 0, m_width - 1 ), clamp<int> ( y, 0, m_height - 1 ) );
    }
    ...
    inline const vec4F_vec &get_pixels() const
    {
        return m_pixels;
    }
    inline vec4F_vec &get_pixels()
    {
        return m_pixels;
    }

    inline const vec4F *get_ptr() const
    {
        return &m_pixels[0];
    }
    inline vec4F *get_ptr()
    {
        return &m_pixels[0];
    }
...
{
    return load_png ( filename.c_str(), img );
}
...
{
    return save_png ( filename.c_str(), img, image_save_flags, grayscale_comp );
}
...
{
    return v.size() ? write_data_to_file ( pFilename, &v[0], v.size() ) : write_data_to_file ( pFilename, "", 0 );
}
...
public:
    ...
    {
        return 1000.0f* get_elapsed_secs();
    }
...
public:
    ...
    {
        return ( uint32_t ) m_values.size() * sizeof ( m_values[0] );
    }
    ...
    {
        return ( *this ) ( clamp<int> ( x, 0, m_width ), clamp<int> ( y, 0, m_height ) );
    }
