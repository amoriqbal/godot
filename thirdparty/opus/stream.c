...
static int op_fseek ( void *_stream,opus_int64 _offset,int _whence )
{
    ...
    if ( _whence==SEEK_CUR ) {
        ...
        pos=_filelengthi64 ( _fileno ( ( FILE * ) _stream ) );
    } else if ( _whence==SEEK_SET ) {
        pos=0;
    } else {
        return -1;
    }
    /*Check for errors or overflow.*/
    if ( pos<0||_offset<-pos||_offset>OP_INT64_MAX-pos ) {
        return -1;
    }
    pos+=_offset;
                   ...
                   void *_stream )
{
    ...
    if ( _path==NULL||_mode==NULL ) {
        ...
        if ( wmode==NULL ) {
            ...
            fp=_wfreopen ( wpath,wmode, ( FILE * ) _stream );
        }
        _ogg_free ( wmode );
        _ogg_free ( wpath );
    }
...
static int op_mem_seek ( void *_stream,opus_int64 _offset,int _whence )
{
    ...
    case SEEK_CUR: {
        /*Check for overflow:*/
        if ( _offset<-pos||_offset>OP_MEM_DIFF_MAX-pos ) {
            return -1;
        }
    ...
    case SEEK_END: {
        ...
        OP_ASSERT ( size>=0 );
        /*Check for overflow:*/
        if ( _offset>size||_offset<size-OP_MEM_DIFF_MAX ) {
            return -1;
        }
