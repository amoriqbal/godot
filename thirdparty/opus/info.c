...
static int op_tags_ensure_capacity ( OpusTags *_tags,size_t _ncomments )
{
    ...
    size_t   size;
    if ( OP_UNLIKELY ( _ncomments>= ( size_t ) INT_MAX ) ) {
        return OP_EFAULT;
    }
    ...
    if ( OP_UNLIKELY ( comment_lengths==NULL ) ) {
        return OP_EFAULT;
    }
    ...
    if ( OP_UNLIKELY ( user_comments==NULL ) ) {
        return OP_EFAULT;
    }
                                  ...
                                  const unsigned char *_data,size_t _len )
{
    ...
    if ( len>0&& ( _data[0]&1 ) ) {
        ...
        if ( _tags!=NULL ) {
            ...
            if ( OP_UNLIKELY ( _tags->user_comments[ncomments]==NULL ) ) {
                return OP_EFAULT;
            }
...
static int opus_tags_copy_impl ( OpusTags *_dst,const OpusTags *_src )
{
    ...
    if ( OP_UNLIKELY ( _dst->vendor==NULL ) ) {
        return OP_EFAULT;
    }
    ...
    for ( ci=0; ci<ncomments; ci++ ) {
        ...
        if ( OP_UNLIKELY ( _dst->user_comments[ci]==NULL ) ) {
            return OP_EFAULT;
        }
    ...
    if ( _src->comment_lengths!=NULL ) {
        ...
        if ( len>0 ) {
            ...
            if ( OP_UNLIKELY ( _dst->user_comments[ncomments]==NULL ) ) {
                return OP_EFAULT;
            }
                                  ...
                                  const unsigned char *_data,int _len )
{
    ...
    int            ncomments;
    int            ret;
    if ( _len<0||_len>0&& ( _data==NULL||! ( _data[0]&1 ) ) ) {
        return OP_EINVAL;
    }
    ncomments=_tags->comments;
    ...
    if ( OP_UNLIKELY ( binary_suffix_data==NULL ) ) {
        return OP_EFAULT;
    }
        ...
        unsigned char *_buf,size_t _buf_sz,size_t _base64_sz )
{
    ...
    for ( i=0; i<_base64_sz; i++ ) {
        ...
        if ( 3*i+1<_buf_sz ) {
            ...
            if ( 3*i+2<_buf_sz ) {
                _buf[3*i+2]= ( unsigned char ) value;
            }
        }
    }
    i=0;
    ...
    if ( mime_type_length==3&&strcmp ( mime_type,"-->" ) ==0 ) {
        ...
        if ( mime_type_length==10
            ...
            if ( op_is_png ( _buf+i,data_length ) ) {
                format=OP_PIC_FORMAT_PNG;
            }
        } else if ( mime_type_length==9
                    &&op_strncasecmp ( mime_type,"image/gif",mime_type_length ) ==0 ) {
            if ( op_is_gif ( _buf+i,data_length ) ) {
                format=OP_PIC_FORMAT_GIF;
            }
            ...
            if ( op_is_jpeg ( _buf+i,data_length ) ) {
                ...
                format=OP_PIC_FORMAT_PNG;
            } else if ( op_is_gif ( _buf+i,data_length ) ) {
                format=OP_PIC_FORMAT_GIF;
            }
        }
...
int opus_picture_tag_parse ( OpusPictureTag *_pic,const char *_tag )
{
    ...
    size_t          tag_length;
    int             ret;
    if ( opus_tagncompare ( "METADATA_BLOCK_PICTURE",22,_tag ) ==0 ) {
        _tag+=23;
    }
