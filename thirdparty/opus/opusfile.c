              ...
              const unsigned char *_initial_data,size_t _initial_bytes )
{
    ...
    if ( memcmp ( _initial_data,"OggS",4 ) !=0 ) {
...
static int op_get_data ( OggOpusFile *_of,int _nbytes )
{
    ...
    if ( OP_LIKELY ( nbytes>0 ) ) {
        ogg_sync_wrote ( &_of->oy,nbytes );
    }
    return nbytes;
}
                                     ...
                                     const ogg_uint32_t *_serialnos,int _nserialnos )
{
    ...
    do {
        ...
        if ( OP_UNLIKELY ( !begin ) &&OP_UNLIKELY ( _offset<0 ) ) {
            return OP_EBADLINK;
        }
                            ...
                            opus_int32 _delta )
{
    ...
    if ( _delta>0 ) {
        /*Adding this amount to the granule position would overflow its 64-bit
           range.*/
        if ( OP_UNLIKELY ( _src_gp<0 ) &&OP_UNLIKELY ( _src_gp>=-1-_delta ) ) {
            return OP_EINVAL;
        }
        ...
        /*Subtracting this amount from the granule position would underflow its
           64-bit range.*/
        if ( _src_gp>=0&&OP_UNLIKELY ( _src_gp<-_delta ) ) {
            return OP_EINVAL;
        }
                             ...
                             ogg_int64_t _gp_a,ogg_int64_t _gp_b )
{
    ...
    if ( OP_UNLIKELY ( gp_a_negative^gp_b_negative ) ) {
        ...
        if ( gp_a_negative ) {
            ...
            if ( OP_UNLIKELY ( OP_INT64_MAX+da<db ) ) {
                return OP_EINVAL;
            }
            *_delta=db-da;
        } else {
            ...
            /*Step 2: Check for overflow.*/
            if ( OP_UNLIKELY ( da<OP_INT64_MIN-db ) ) {
                return OP_EINVAL;
            }
            *_delta=da+db;
        }
                                        ...
                                        OggOpusLink *_link,ogg_page *_og )
{
    ...
    do {
        ...
        if ( serialno!= ( ogg_uint32_t ) ogg_page_serialno ( _og ) ) {
            continue;
        }
        ogg_stream_pagein ( &_of->os,_og );
    ...
    for ( pi=0; pi<op_count; pi++ ) {
        if ( cur_page_eos ) {
            ...
            if ( diff>0 ) {
                ...
                if ( OP_UNLIKELY ( diff>durations[pi] ) ) {
                    break;
                }
                                        ...
                                        ogg_uint32_t **_serialnos,int *_nserialnos,int *_cserialnos )
{
    ...
    for ( ;; ) {
        ...
        for ( sri=0; sri<nsr; sri++ ) {
            if ( op_lookup_serialno ( _sr[sri].serialno,serialnos,nserialnos ) ) {
                break;
            }
        }
        ...
        if ( nlinks>1 ) {
            ...
            if ( OP_LIKELY ( last_offset>_searched-avg_link_size )
                ...
                if ( OP_LIKELY ( bisect<upper_limit ) ) {
...
static int op_open_seekable2_impl ( OggOpusFile *_of )
{
    ...
    if ( OP_UNLIKELY ( _of->end<data_offset ) ) {
        return OP_EBADLINK;
    }
...
static void op_clear ( OggOpusFile *_of )
{
    ...
    if ( _of->callbacks.close!=NULL ) {
        ( *_of->callbacks.close ) ( _of->source );
    }
}
                      ...
                      const unsigned char *_initial_data,size_t _initial_bytes )
{
    ...
    *&_of->callbacks=*_cb;
    ...
    if ( seekable ) {
        opus_int64 pos;
        if ( OP_UNLIKELY ( _of->callbacks.tell==NULL ) ) {
            return OP_EINVAL;
        }
        ...
        if ( OP_UNLIKELY ( pos!= ( opus_int64 ) _initial_bytes ) ) {
            return OP_EINVAL;
        }
    }
...
{
    int ret;
    if ( OP_UNLIKELY ( _of->ready_state!=OP_PARTOPEN ) ) {
        return OP_EINVAL;
    }
    ret=op_open2 ( _of );
...
opus_int64 op_raw_total ( const OggOpusFile *_of,int _li )
{
    ...
    if ( _li<0 ) {
        return _of->end-_of->links[0].offset;
    }
...
const OpusTags *op_tags ( const OggOpusFile *_of,int _li )
{
    ...
    if ( !_of->seekable ) {
        ...
        _li=_of->ready_state>=OP_STREAMSET?_of->cur_link:0;
    }
...
opus_int32 op_bitrate_instant ( OggOpusFile *_of )
{
    ...
    if ( OP_UNLIKELY ( samples_tracked==0 ) ) {
        return OP_FALSE;
    }
...
int op_raw_seek ( OggOpusFile *_of,opus_int64 _pos )
{
    ...
    if ( OP_UNLIKELY ( !_of->seekable ) ) {
        return OP_ENOSEEK;
    }
    if ( OP_UNLIKELY ( _pos<0 ) ||OP_UNLIKELY ( _pos>_of->end ) ) {
        return OP_EINVAL;
    }
    ...
    while ( begin<end ) {
        ...
        while ( begin<end ) {
            ...
            if ( page_offset<0 ) {
                ...
                if ( serialno!= ( ogg_uint32_t ) ogg_page_serialno ( &og ) ) {
                    continue;
                }
    ...
    if ( !buffering ) {
        ...
        if ( best_start<best ) {
            ...
            if ( page_offset<0 ) {
                ...
                if ( OP_UNLIKELY ( page_offset<OP_FALSE ) ) {
                    return ( int ) page_offset;
                }
                if ( OP_UNLIKELY ( page_offset!=best_start ) ) {
                    return OP_EBADLINK;
                }
            }
            op_buffer_continued_data ( _of,&og );
...
int op_pcm_seek ( OggOpusFile *_of,ogg_int64_t _pcm_offset )
{
    ...
    if ( OP_UNLIKELY ( _pcm_offset<0 ) ) {
        return OP_EINVAL;
    }
    target_gp=op_get_granulepos ( _of,_pcm_offset,&li );
    if ( OP_UNLIKELY ( target_gp==-1 ) ) {
        return OP_EINVAL;
    }
    link=_of->links+li;
    ...
    if ( _pcm_offset<=link->head.pre_skip ) {
        ...
        skip=OP_MAX ( _pcm_offset-80*48,0 );
    }
                                   ...
                                   op_sample *_src,int _nsamples,int _nchannels )
{
    ...
    ( void ) _of;
    dst= ( float * ) _dst;
    if ( OP_UNLIKELY ( _nsamples*_nchannels>_dst_sz ) ) {
