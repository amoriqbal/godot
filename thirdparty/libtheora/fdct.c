                         ...
                         ogg_int16_t _y[64],const ogg_int16_t _x[64] )
{
    ...
    in=w;
    out=_y;
    if ( cext==NULL ) for ( ci=0; ci<8; ci++ ) {
            oc_fdct8 ( out+ ( ci<<3 ),in+ci );
        } else for ( ci=0; ci<8; ci++ ) if ( rmask& ( 1<<ci ) ) {
                oc_fdct8_ext ( out+ ( ci<<3 ),in+ci,cext );
            }
    /*Transform the rows.
    ...
    in=_y;
    out=w;
    if ( rext==NULL ) for ( ri=0; ri<8; ri++ ) {
            oc_fdct8 ( out+ ( ri<<3 ),in+ri );
        } else for ( ri=0; ri<8; ri++ ) {
            oc_fdct8_ext ( out+ ( ri<<3 ),in+ri,rext );
        }
    /*Round the result back to the external working precision (which is still
       scaled by four relative to the orthogonal result).
      TODO: We should just update the external working precision.*/
